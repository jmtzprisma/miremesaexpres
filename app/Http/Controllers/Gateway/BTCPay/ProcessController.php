    ...
    public function ipn() {
        ...
        if ($rawPostData) {
            ...
            if (!isset($signature) || !$this->validWebhookRequest($signature, $rawPostData, $gatewayParameters->secret_code->value)) {
                ...
                $adminNotification->title     = trans('Webhook request validation failed.');
            ...
            try {
                ...
                if (!isset($postData->invoiceId)) {
                    $adminNotification            = new AdminNotification();
                    $adminNotification->user_id   = 0;
                    $adminNotification->title     = trans('No BTCPay invoiceId provided, aborting.');
                    $adminNotification->click_url = '#';
                    $adminNotification->save();
                    return false;
                }
                ...
                $adminNotification            = new AdminNotification();
                $adminNotification->user_id   = 0;
                $adminNotification->title     = trans('Error decoding webhook payload') . ': ' . $e->getMessage();
                $adminNotification->click_url = '#';
                $adminNotification->save();
                return false;
            }
    ...
    public function processPayment($deposit, $webhookData) {
        if ($webhookData->type == 'InvoicePaymentSettled') {
            if ($webhookData->afterExpiration) {
                $adminNotification            = new AdminNotification();
                $adminNotification->user_id   = 0;
                $adminNotification->title     = trans('Payment expired for trx') . ' ' . $deposit->trx;
                $adminNotification->click_url = '#';
                $adminNotification->save();
                return false;
            } else {
                if ($webhookData->payment->status == 'Settled') {
                    ...
                    $adminNotification            = new AdminNotification();
                    $adminNotification->user_id   = 0;
                    $adminNotification->title     = trans('Amount is not fully paid for trx') . ' ' . $deposit->trx;
                    $adminNotification->click_url = '#';
                    $adminNotification->save();
                    return false;
                }
