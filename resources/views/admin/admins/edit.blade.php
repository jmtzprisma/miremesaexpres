    ...
    <div class="row justify-content-center">
        <div class="col-12">
            <div class="card b-radius--5 overflow-hidden">
                <div class="card-body">
                    <form action="{{ route('admin.adm.update', $user->id) }}" method="POST" class="verify-gcaptcha row">
                        @csrf
                        ...
                        <div class="form-group col-md-6">
                            ...
                            <input id="username" type="text" class="form-control checkUser" name="username" value="{{ $user->username }}" required>
                            <small class="text-danger usernameExist"></small>
                        </div>
                        <div class="form-group col-md-6">
                            <label for="email">@lang('E-Mail Address')</label>
                            <input id="email" type="email" class="form-control checkUser" name="email" value="{{ $user->email }}" required>
                        </div>
                        <div class="form-group col-md-6">
                            <label for="role_id">@lang('Rol')</label>
                            <select name="role_id" class="form-control">
                                @foreach (\App\Models\Roles::get() as $itm)
                                    <option value="{{ $itm->id }}" @if($user->role_id == $itm->id) selected @endif>{{ __($itm->name) }}</option>
                                @endforeach
                            </select>
                        </div>
                        <hr>
                        <h6>Si desea cambiar la contraseña escribala, sino solo dejela en blanco</h6>
                        <div class="form-group col-md-6">
