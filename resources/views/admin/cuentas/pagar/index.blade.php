...
@section('panel')
                        ...
                        <table class="table table-responsive-md">
                            ...
                            <tbody>
                                @forelse($cuentas as $cuenta)
                                    <tr>
                                        <td><span class="small">{{ showDateTime($cuenta->created_at) }}</span></td>
                                        <td><span class="text--muted fw-bold">{{ $cuenta->id }}</span></td>
                                        <td><span class="small">{{ $cuenta->bank->name }}</span></td>
                                        <td><span class="small">{{ $cuenta->concepto }}</span></td>
                                        <td><span class="small">{{ $cuenta->proveedor }}</span></td>
                                        <td><span class="small">{{ showAmount($cuenta->amount_currency_local) }} {{$cuenta->bank->currency}}</span></td>
                                        ...
                                        <td><span class="small">{{ showAmount($cuenta->amount_currency_local - $cuenta->sumPagos()) }} {{$cuenta->bank->currency}}</span></td>
                                        <td>
                                            <span class="small">
    ...
    <div class="modal fade" id="nuevaCuentaPagar" role="dialog" tabindex="-1">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form action="{{ route('admin.cuentas.pagar.store') }}" method="POST">
                    @csrf
    ...
    <div class="modal fade" id="agregarPagoModal" role="dialog" tabindex="-1">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form action="{{ route('admin.cuentas.save_cp_payment') }}" method="POST">
                    @csrf
    ...
    <div class="modal fade" id="cancelarPagoModal" role="dialog" tabindex="-1">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form action="{{ route('admin.cuentas.cancel_cuenta_pagar') }}" method="POST">
                    @csrf
    ...
    <div class="modal fade" id="cancelarcxpPagoModal" role="dialog" tabindex="-1">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">@lang('Cancelar pago de cuenta por pagar')</h5>
                    <button aria-label="Close" class="close" data-bs-dismiss="modal" type="button">
                        <i class="las la-times"></i>
                    </button>
                </div>
                <form action="{{ route('admin.cuentas.cancel_cxp_pago') }}" method="POST">
                    @csrf
                    <input type="hidden" name="cxpPagoId" id="cxpPagoIdCancelar">
                    <div class="modal-body">
                        <div class="form-group">
    ...
    $(document).ready(function() {
        $(".btn-lista-pagos").on("click", function(){
            ...
            $.ajax({
                ...
                type: "POST",
                url: "{{ route('admin.cuentas.cp.pagos') }}",
                data: {
                    'cuenta_id': cuentaId
                },
        ...
        $("#status").on("change", function(){
            window.location.href = "{{route('admin.cuentas.index.pagar')}}/"+$("#status").val()
        })
